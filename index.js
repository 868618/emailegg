const {createConnection, connect} = require('net');
...
module.exports = function (options) {
    ...
    const localAddress = options.localAddress || '127.0.0.1';
    /*
    ...
    function sendToSMTP (domain, srcHost, from, recipients, body, cb) {
        ...
        connectMx(domain, function (err, sock, exchange) {
            if (err) {
            ...
            function response (code, msg) {
                switch (code) {
                    ...
                    case 221: // bye
                        // callback(new Error('SMTP code:' + code + ', from:' + from +', to:' + recipients.toString() + ', domain:' + domain + ', msg:' + msg + ', mx:' + exchange + '\r\n'))
                        const info = {
                            code,
                            from,
                            domain,
                        ...
                        callback(null, `Success: SMTP code:${code}, from:${from}, to:${recipients.toString()}, domain:${domain}, msg:${msg.replace('\r\n', '')}, mx:${exchange}`);
                        break;
                    case 235: // verify ok
